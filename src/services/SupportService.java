...
public class SupportService implements IDao<Support> {
    ...
    public List<Support> findByProfesseur(int professeurId) {
        List<Support> supports = new ArrayList<>();
        String req = "SELECT s.* FROM support s JOIN ajoutsupport a ON s.id = a.support_id WHERE a.professeur_id = ?";
        try (PreparedStatement ps = connexion.getCn().prepareStatement(req)) {
